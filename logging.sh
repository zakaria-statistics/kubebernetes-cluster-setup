...
LOGFILE="/var/log/kubernetes_inspection1.log"
...
# Function to log command output (stdout and stderr)
log_command() {
    local command="$1"
    local label="$2"
    echo "---------------------------------------------" | tee -a $LOGFILE
...
for pod in kube-apiserver kube-controller-manager etcd; do
    log_command "kubectl logs -n kube-system -l component=$pod --tail=10" "Logs for $pod"
done
echo "---------------------------------------------" | tee -a $LOGFILE

# 3. Checking the status of nodes
log_command "kubectl get nodes" "Node status check"
echo "---------------------------------------------" | tee -a $LOGFILE

# 4. Checking services
log_command "kubectl get svc -n kube-system" "Checking services in kube-system namespace"
echo "---------------------------------------------" | tee -a $LOGFILE

# 5. Checking deployments
log_command "kubectl get deployments -n kube-system" "Checking deployments in kube-system namespace"
echo "---------------------------------------------" | tee -a $LOGFILE

# 6. Checking pods in kube-system namespace
log_command "kubectl get pods -n kube-system" "Checking pods in kube-system namespace"
echo "---------------------------------------------" | tee -a $LOGFILE

# 7. Checking pods in default namespace
log_command "kubectl get pods" "Checking pods in default namespace"
echo "---------------------------------------------" | tee -a $LOGFILE
...
if [ $? -ne 0 ]; then
    echo "No nodes are in Ready state." | tee -a $LOGFILE
else
...
log_command "kubectl get netpol" "Checking network policies"
echo "---------------------------------------------" | tee -a $LOGFILE
...
log_command "kubectl describe nodes | grep -i taints" "Checking for taints on nodes"
echo "---------------------------------------------" | tee -a $LOGFILE
